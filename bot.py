...
class SupportBot:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard.append([InlineKeyboardButton("🚪 Выйти", url="https://t.me/ChatFlat_Bot")])
...
⏰ <b>Время создания:</b> {datetime.now().strftime('%d.%m.%Y %H:%M')}
        ...
        await update.message.reply_text(
            ...
            f"💬 <b>Ожидайте ответа от поддержки</b>",
            parse_mode='HTML',
        ...
        if open_tickets_list:
            for ticket_id in open_tickets_list[:8]:
                ...
                if ticket_data:
                    ...
                    btn_text = f"🎫 {time_ago} - {ticket_data['subject'][:25]}..."
...
🎫 <b>Открытые тикеты:</b>
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "create_ticket":
            ...
            keyboard = [[InlineKeyboardButton("❌ Отмена", callback_data=f"user_view_{ticket_id}")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(
...
• Активных пользователей: {self.get_active_users_count()}
    ...
    async def show_admin_users(self, query):
        ...
        for user_id, tickets_list in list(self.user_tickets.items())[:15]:
            ...
            users_text += f"   🎫 Тикетов: {len(user_tickets)} (🔓{open_count} ✅{closed_count})\n"
